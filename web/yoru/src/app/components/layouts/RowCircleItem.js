    ...
    return(
        <div className={props.className} 
            ...
            <div className={`w-full h-full flex flex-col space-y-2 items-center transition-all relative overflow-hidden`}>
                <img alt={`img`} src={`http://${props.src}`} className={` h-2/3  ${props.rounded?`rounded-full`:``} `}/>
               <div className={`w-full flex flex-col`}>
                    <div className={`text-2xl`}>{props.name}</div>
                    {props.rounded?<div>艺人</div>:<div>歌单</div>} 
               </div>
              <GreenAdvance   
              className={`absolute bg-white transition rounded-full bottom-0 right-4  hover:cursor-pointer ${state.is_hover?`-translate-y-12`:`translate-y-12`}`}
               w={`48`} h={`48`}
